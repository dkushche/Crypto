...
def check_params(p_value, q_value):
    ...
    if (p_value - 1) % q_value != 0:
        raise ValueError(f"P - 1 is not devisable by Q {p_value - 1} % {q_value} != 0")
...
def dsa_sign(data, p_value, q_value, x_value, k_value, g_value):
    ...
    s_value = crypto_tools.inverse_modulo_numb(k_value, q_value) * ((data + x_value * r_value) % q_value)
...
def dsa_verify(data, p_value, q_value, g_value, open_key):
    ...
    v_value = (pow(g_value, u1_value) * pow(open_key, u2_value) % p_value) % q_value
