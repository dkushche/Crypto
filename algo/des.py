...
def key_extension(key):
    ...
    extended_key = des_mutation(key_mutation_table, extended_key, len(extended_key))
...
def des_secret_func(val, now_round, block_size, extended_key):
    ...
    val = des_mutation(extension_table, val, int(block_size / 2))
    ...
    round_key = des_mutation(key_regression_table, shifted_key, len(shifted_key))
...
def des_processing(data, key, encrypt):
    ...
    data = des_mutation(start_permutation_table, data, block_size)
