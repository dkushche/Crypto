...
class elliptic_point:
    ...
    def __neg__(self):
        return elliptic_point(self.x, -self.y % elliptic_point.elliptic_curve["p"])
    ...
    def __add__(self, other):
        if isinstance(other, elliptic_point):
            ...
            if self == other:
                m = (3 * pow(self.x, 2) + elliptic_point.elliptic_curve["a"]) *\
                    inverse_modulo_numb(
                        2 * self.y,
                ...
                if self.x - other.x != 0:
                    m = (self.y - other.y) * inverse_modulo_numb(
                        self.x - other.x,
                        elliptic_point.elliptic_curve["p"]
                        ) % elliptic_point.elliptic_curve["p"]
                else:
            ...
            new_x = (pow(m, 2) - self.x - other.x) % elliptic_point.elliptic_curve["p"]
            new_y = (self.y + m * (new_x - self.x)) % elliptic_point.elliptic_curve["p"]
            return -elliptic_point(new_x, new_y)
        else:
